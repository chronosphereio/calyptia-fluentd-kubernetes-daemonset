...
By default, these images use containerd/cri-o log format. To parse such logs, this daemonset use [`cri` parser](https://github.com/fluent/fluent-plugin-parser-cri).
On the other hand, you can use `json` parser for `/var/log/containers/` when you parse docker log format files because docker generates json formatted logs.

You can use `json` parser by overwriting `tail_container_parse.conf` via ConfigMap.
